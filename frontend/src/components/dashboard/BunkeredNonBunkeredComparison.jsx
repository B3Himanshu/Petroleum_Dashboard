...
export const BunkeredNonBunkeredComparison = ({ startDate, endDate, siteIds }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log('📦 [BunkeredComparison] Fetching data for months:', months);
        
        // Single month - redirect to regular donut chart
        if (months.length === 1) {
          setMonthlyData([]);
          return;
        }
        ...
        const monthlyResults = await Promise.all(
          months.map(async (m) => {
            ...
            const data = await dashboardAPI.getPetrolFuelVolumeBreakdown(monthStart, monthEnd, siteIds);
            
            const breakdown = data?.breakdown || [];
            const bunkered = breakdown.find(b => b.name === "Bunkered")?.volume || 0;
            const nonBunkered = breakdown.find(b => b.name === "Non-Bunkered")?.volume || 0;
        ...
        console.log('📦 [BunkeredComparison] Monthly results:', monthlyResults);
        setMonthlyData(monthlyResults);
      } catch (err) {
        console.error('❌ [BunkeredComparison] Error fetching data:', err);
      ...
      tooltip: {
        ...
        callbacks: {
          label: function(context) {
            ...
            return `${context.dataset.label}: ${value.toFixed(2)}M litres`;
          }
        }
      },
    },
