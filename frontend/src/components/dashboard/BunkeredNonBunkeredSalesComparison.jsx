...
export const BunkeredNonBunkeredSalesComparison = ({ startDate, endDate, siteIds }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log('💰 [SalesComparison] Fetching data for months:', months);
        ...
        const monthlyResults = await Promise.all(
          months.map(async (m) => {
            ...
            const data = await dashboardAPI.getPetrolNetSalesBreakdown(monthStart, monthEnd);
            
            const breakdown = data?.breakdown || [];
            const bunkered = breakdown.find(b => b.name === "Bunkered")?.sales || 0;
            const nonBunkered = breakdown.find(b => b.name === "Non-Bunkered")?.sales || 0;
        ...
        console.log('💰 [SalesComparison] Monthly results:', monthlyResults);
        setMonthlyData(monthlyResults);
      } catch (err) {
        console.error('❌ [SalesComparison] Error fetching data:', err);
