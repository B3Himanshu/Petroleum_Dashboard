...
export const BunkeredNonBunkeredProfitChart = ({ startDate, endDate, onBreakdown }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        setError(null);
        console.log('📈 [BunkeredNonBunkeredProfitChart] Fetching profit data:', { startDate, endDate });
        ...
        const bunkeredProfit = bunkeredData?.profit || 0;
        const nonBunkeredProfit = nonBunkeredData?.profit || 0;
        const totalProfit = bunkeredProfit + nonBunkeredProfit;
        
        const bunkeredPercentage = totalProfit > 0 ? Math.round((bunkeredProfit / totalProfit) * 100) : 0;
        const nonBunkeredPercentage = totalProfit > 0 ? Math.round((nonBunkeredProfit / totalProfit) * 100) : 0;
        
        const profitData = [
          {
            name: "Bunkered",
            value: bunkeredProfit,
            percentage: bunkeredPercentage,
            color: "#ef4444"
          },
          {
            name: "Non-Bunkered",
            value: nonBunkeredProfit,
        ...
        console.error('❌ [BunkeredNonBunkeredProfitChart] Error fetching data:', err);
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function(context) {
            ...
            return [
              `Profit: £${(value / 1000000).toFixed(2)}M`,
        ...
        <div className="flex items-start justify-between">
          <div>
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              Bunkered vs Non-Bunkered Profit
            </CardTitle>
            <CardDescription>
              Profit distribution by fuel supply type
            </CardDescription>
          </div>
          {onBreakdown && (
            <button
