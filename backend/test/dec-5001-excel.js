/**
 * Export December 2025 – nominal code 5001 (Diesel-Purchases) only – to Excel.
 * Includes both positive and negative amounts (no ABS). Sum = sum of all transaction amounts.
 *
 * Run from backend: node test/dec-5001-excel.js
 * Output: backend/exports/dec-5001-YYYY-MM-DD-HHMMSS.xlsx
 *
 * Default: Dec 2025. Optional env: DEC_5001_YEAR=2025, DEC_5001_MONTH=12, DEC_5001_EXPECTED=4876956.08 (reference value to show difference).
 */
...
const year = parseInt(process.env.DEC_5001_YEAR || '2025', 10);
const month = parseInt(process.env.DEC_5001_MONTH || '12', 10);
...
async function run() {
  console.log(`Dec ${year} – nominal code 5001 (Diesel-Purchases) export`);
  ...
  const res = await query(
    `SELECT id, nominal_code, dept_number, transaction_date, amount, details, created_at
     FROM transactions
     WHERE TRIM(nominal_code::text) = '5001'
  ...
  // Reference expected total (real/Sage). Default 4876956.08; override with env DEC_5001_EXPECTED.
  const expectedRaw = process.env.DEC_5001_EXPECTED;
  const parsed = expectedRaw != null && expectedRaw !== '' ? parseFloat(expectedRaw) : NaN;
  const expected = Number.isNaN(parsed) ? 4876956.08 : parsed;
  const difference = sumTotal - expected;
  // When DB stores purchases as positive but Sage uses negative, "for profit" = -sumTotal
  ...
  if (rows.length === 0) {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), '5001_All');
  }

  const summaryRows = [
    ['5001 - Diesel-Purchases summary (simple vs ABS)'],
    ['Period', `${startDate} to ${endDate}`],
  ...
  const filename = `dec-5001-${timestamp()}.xlsx`;
