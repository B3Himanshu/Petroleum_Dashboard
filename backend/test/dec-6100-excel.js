/**
 * Export December 2025 – nominal code 6100 (Fuel Commissions) only – to Excel.
 * Includes both positive and negative amounts (no ABS). Sum = sum of all transaction amounts.
 *
 * Run from backend: node test/dec-6100-excel.js
 * Output: backend/exports/dec-6100-YYYY-MM-DD-HHMMSS.xlsx
 *
 * Default: Dec 2025. Optional env: DEC_6100_YEAR=2025, DEC_6100_MONTH=12, DEC_6100_EXPECTED=6971.01 (reference value to show difference).
 */
...
const year = parseInt(process.env.DEC_6100_YEAR || '2025', 10);
const month = parseInt(process.env.DEC_6100_MONTH || '12', 10);
...
async function run() {
  console.log(`Dec ${year} – nominal code 6100 (Fuel Commissions) export`);
  ...
  const res = await query(
    `SELECT id, nominal_code, dept_number, transaction_date, amount, details, created_at
     FROM transactions
     WHERE TRIM(nominal_code::text) = '6100'
  ...
  const expectedRaw = process.env.DEC_6100_EXPECTED;
  const parsed = expectedRaw != null && expectedRaw !== '' ? parseFloat(expectedRaw) : NaN;
  const expected = Number.isNaN(parsed) ? 6971.01 : parsed;
  ...
  if (rows.length === 0) {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), '6100_All');
  }

  const summaryRows = [
    ['6100 - Fuel Commissions summary (simple vs ABS)'],
    ['Period', `${startDate} to ${endDate}`],
  ...
  const filename = `dec-6100-${timestamp()}.xlsx`;
